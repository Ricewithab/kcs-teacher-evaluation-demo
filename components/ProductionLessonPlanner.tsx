...
export function ProductionLessonPlanner(){
 ...
 const contexts=useMemo(()=>{
  ...
  if(user&&["teacher","staff"].includes(user.systemRole))result.push({key:"new",evaluationId:null,teacherId:user.staffId,label:"New general lesson plan",plan:null});
  return result;
 },[evaluations,plans,state?.staff,user?.staffId,user?.systemRole]);
 ...
 useEffect(()=>{
  ...
  const selected=requested??ownEvaluation??ownPlan??contexts[0];
 ...
 if(!contexts.length)return <div className="page"><header className="page-head"><div><span className="eyebrow">Lesson planning</span><h1>Lesson plans</h1><p>No lesson-plan context is available in your current scope.</p></div></header></div>;
