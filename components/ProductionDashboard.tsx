...
function Teacher({user,state}:{user:any;state:any}){
 ...
 return <div className="page"><PageHead eyebrow="My teaching" title={`Hello, ${user.name}`} text="Your required evaluations, preparation, feedback and development actions."/>
  <section className="hero"><div><span className="hero-kicker"><CalendarClock size={15}/> Next evaluation</span>{scheduled?<><h2>{scheduled.window_label}</h2><p>{formatDate(scheduled.scheduled_at)} · {scheduled.class_name??"Class to be confirmed"}</p><div className="button-row"><Link className="button primary" href={scheduled.evaluation_id?`/evaluations/${scheduled.evaluation_id}`:"/evaluations"}>Open evaluation <ArrowRight size={15}/></Link><Link className="button secondary" href="/lesson-planning">{plan?"Open lesson plan":"Create lesson plan"}</Link></div></>:<><h2>No observation currently scheduled</h2><p>Your line manager can schedule a required observation from the evaluation centre.</p><Link className="button secondary" href="/evaluations">View annual requirements</Link></>}</div><div className="progress-ring"><strong>{completed}/{requirements.length||0}</strong><span>annual requirements</span></div></section>
...
function Manager({user,state}:{user:any;state:any}){
 const staff=(state.staff??[]).filter((s:any)=>s.id!==user.staffId&&s.evaluation_eligible&&s.active);
 ...
 return <div className="page"><PageHead eyebrow="Department leadership" title={user.department} text="Required observations and next actions for staff in your reporting hierarchy."/>
  ...
  <section className="card"><CardTitle title="My team" subtitle="Status is calculated from the Master-defined annual requirements."/><TeamTable staff={staff} requirements={reqs}/></section>
 </div>
}

function Leadership({user,state,division=false}:{user:any;state:any;division?:boolean}){
 const reqs=state.requirements??[];
 const staff=(state.staff??[]).filter((s:any)=>s.evaluation_eligible&&s.active);
 ...
 return <div className="page"><PageHead eyebrow={division?user.division:"Whole school · Master"} title="Teacher Evaluation" text="Live progress against the school’s published evaluation requirements." actions={!division?<Link className="button primary" href="/master">Configure system</Link>:undefined}/>
