...
async function manager(request:Request){const identity=await getAuthenticatedIdentity(request);if(!identity)return{identity:null,response:unauthorized()};if(!canManageHierarchy(identity))return{identity:null,response:forbidden()};return{identity,response:null}}
...
export async function POST(request:Request){
 try{
  ...
  const access=await manager(request);if(access.response||!access.identity)return access.response!;
  ...
  await recordAudit(access.identity.staffId,"staff.created","staff",id,null,after);
...
export async function PATCH(request:Request){
 try{
  ...
  const after=await database().prepare("SELECT * FROM staff WHERE id = ?").bind(String(body.id)).first();await recordAudit(access.identity.staffId,"staff.updated","staff",String(body.id),before,after);
  return Response.json({ok:true,staff:after});
