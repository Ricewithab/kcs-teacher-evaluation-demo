...
import { CalendarRange, GitBranch, GripVertical, History, Save, Settings2, ShieldCheck, UsersRound } from "lucide-react";
...
export function MasterConfiguration(){
 ...
 const [managerByStaff,setManagerByStaff]=useState<Record<string,string>>({});
 const [reportingStaff,setReportingStaff]=useState("s47");
 ...
 const reportingCandidates=STAFF.filter(member=>member.status==="Active"&&["master","division","manager"].includes(member.role)&&member.id!==reportingStaff);
 ...
 useEffect(()=>{
  ...
  fetch(apiPath("/api/state"),{cache:"no-store"})
   ...
   .then(data=>{
    ...
    if(Array.isArray(data.reportingLines))setManagerByStaff(Object.fromEntries(data.reportingLines.filter((line:any)=>line.relationship==="primary"&&line.manager_id).map((line:any)=>[line.staff_id,line.manager_id])));
 ...
 async function saveReportingLine(){
  setReportingSaving(true);setReportingSaved(false);
  try{
   const managerId=managerByStaff[reportingStaff]??"";
   const response=await fetch(apiPath("/api/reporting-lines"),{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({actorId:"s1",staffId:reportingStaff,managerId:managerId||null})});
   if(!response.ok)throw new Error("Unable to save reporting line");
   setReportingSaved(true);await loadAudit();
  }catch(error){console.error(error);window.alert("The reporting line could not be saved.")}
  finally{setReportingSaving(false)}
 }
 ...
 <section className="card"><div className="card-title"><div><h2>Organisation & management lines</h2><p>The roster is initially inferred from the staff list. Master can then drag staff between departments to correct the structure.</p></div><UsersRound/></div><div className="org-builder">{hsDepartments.map(dept=><div className="org-column" key={dept} onDragOver={e=>e.preventDefault()} onDrop={()=>drop(dept)}><div className="org-column-head"><strong>{dept}</strong><span>{buckets[dept]?.length??0} staff</span></div>{(buckets[dept]??[]).map(id=>{const person=STAFF.find(s=>s.id===id)!;return <Person key={id} person={person} onDrag={()=>setDragged(id)}/>})}</div>)}</div><p className="source-note">Moving a staff member persists their department in D1 and updates the inferred primary manager when the destination has a department lead.</p></section>
 <section className="card form-card"><div className="card-title"><div><h2>Primary reporting line override</h2><p>Master can explicitly set who a staff member reports to. Dashboard scope and future permissions can derive from this relationship.</p></div><GitBranch/></div><div className="form-grid"><label>Staff member<select value={reportingStaff} onChange={e=>{setReportingStaff(e.target.value);setReportingSaved(false)}}>{STAFF.filter(member=>member.status==="Active"&&member.id!=="s1").map(member=><option key={member.id} value={member.id}>{member.name} · {member.position}</option>)}</select></label><label>Primary manager<select value={managerByStaff[reportingStaff]??""} onChange={e=>{setManagerByStaff(current=>({...current,[reportingStaff]:e.target.value}));setReportingSaved(false)}}><option value="">No primary manager</option>{reportingCandidates.map(manager=><option key={manager.id} value={manager.id}>{manager.name} · {manager.position}</option>)}</select></label></div><div className="button-row"><button className="button secondary" onClick={saveReportingLine} disabled={reportingSaving}>{reportingSaving?"Saving…":reportingSaved?"Reporting line saved":"Save reporting line"}</button></div></section>
