...
export function MasterConfiguration(){
 ...
 async function save(){
  ...
  try{
   const response=await fetch(apiPath("/api/framework"),{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({actorId:"s1",observationsRequired:required,lessonPlanRequired,feedbackDueDays:feedbackDays,reflectionDueDays,developmentGoalRequired,followUpRequired})});
   if(!response.ok)throw new Error("Unable to save framework");
   setSaved(true);
